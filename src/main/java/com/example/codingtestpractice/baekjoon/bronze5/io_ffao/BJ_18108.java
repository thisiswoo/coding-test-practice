...
public class BJ_18108 {
    public static void main(String[] args) throws IOException {
        int buddistYear = getBuddistYearFromUser();
        int gregorianYear = buddistYearToGregorianYear(buddistYear);
        System.out.println(gregorianYear);
    }

    private static int getBuddistYearFromUser() throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        String s = br.readLine();
        int buddistYear = Integer.parseInt(s);
        return buddistYear;
    }

    private static int buddistYearToGregorianYear(int buddistYear) {
        return buddistYear - 543;
    }
}
